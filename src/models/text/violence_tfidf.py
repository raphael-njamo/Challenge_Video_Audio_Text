...
if __name__ == '__main__':
    ...
    labels = pd.read_csv('data/external/annotation.csv',
                         usecols=['Sequence', 'Violent'])
    ...
    X_train, X_test, y_train, y_test = train_test_split(data.drop(
        ['Sequence'], axis=1), labels['Violent'], test_size=0.33, random_state=42)
    ...
    for i, (fit_idx, val_idx) in tqdm(enumerate(CV.split(X_train))):
        ...
        if verbose:
            print(
                f'accuracy_score fit pour le fold {i+1} : {accuracy_score(pred_fit,Y_fit):.3f}')
            print(
                f'accuracy_score val pour le fold fold {i+1} : {accuracy_score(pred_val,Y_val):.3f}')
    ...
    print(
        f'accuracy_score score pour le fit :{np.mean(fit_score):.3f} ± {np.std(fit_score):.3f}')
    print(
        f'accuracy_score score pour le val :{np.mean(val_score):.3f} ± {np.std(val_score):.3f}')
